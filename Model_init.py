...
def model_train(input_shape, filepath, model_save_path):
    ...
    # # 模型参数保存
    # papa = lstm_model.trainable_variables
    # # print(papa)
...
"""
    # 重载模型训练
    lstm_model2 = load_model(model_save_path)
    N = test_x.shape[0]  # 测试的条数
    predict = []
    label = []
    for start, end in zip(range(0, N, 1), range(1, N + 1, 1)):
        sentence = [inverse_word_dictionary[i] for i in test_x[start] if i != 0]
        y_predict = lstm_model2.predict(test_x[start:end])
        label_predict = output_dictionary[np.argmax(y_predict[0])]
        label_true = output_dictionary[np.argmax(test_y[start:end])]
        # print(''.join(sentence), label_true, label_predict) # 输出预测结果
        predict.append(label_predict)
    ...
    acc = accuracy_score(predict, label)  # 预测准确率
    print('重载模型在测试集上的准确率为: %s.' % acc)

    # 再训练
    lstm_model2.fit(train_x, train_y)
    for start, end in zip(range(0, N, 1), range(1, N + 1, 1)):
        sentence = [inverse_word_dictionary[i] for i in test_x[start] if i != 0]
        y_predict = lstm_model2.predict(test_x[start:end])
        label_predict = output_dictionary[np.argmax(y_predict[0])]
    ...
    acc = accuracy_score(predict, label)  # 预测准确率
    print('再训练模型在测试集上的准确率为: %s.' % acc)
"""

def model_init():
    filepath = './online_shopping_cloth2.csv'
    input_shape = 57
    model_save_path = './cloth_model_0.h5'
